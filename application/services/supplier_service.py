...
class SupplierService():
    ...
    def getAllSupplier(self):
        url = f"{self.base_address}/api/suppliers"
    ...
    def getSupplierById(self, supplierId:str):
        url = f"{self.base_address}/api/suppliers/{supplierId}"
        res = requests.get(url=url)
        res.raise_for_status()
        data = res.json()
        return data.get("documents", [])
    ...
    def activateSupplier(self, supplierId:str, formData:dict):
        url = f"{self.base_address}/api/suppliers_activate/{supplierId}"
        res = requests.put(url=url, data=formData)
        res.raise_for_status()
        return res.json()

    def deactivateSupplier(self, supplierId:str, fData:dict):
        url = f"{self.base_address}/api/suppliers_deactivate/{supplierId}"
        res = requests.put(url=url, data=fData)
        res.raise_for_status()
        return res.json()

    def getActiveSupplier(self):
        url = f"{self.base_address}/api/active_suppliers"
        res = requests.get(url=url)
        res.raise_for_status()
        data = res.json()
        return data.get("documents", [])